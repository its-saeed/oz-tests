  ...
  describe('sendValue', function () {
    ...
    context('when sender contract has no funds', function () {
      it('sends 0 wei', async function () {
        console.log("****************");
        console.log(await ethers.provider.getBalance(recipient));
        await this.mock.sendValue(other, 0);
        console.log("****************");
        console.log(await ethers.provider.getBalance(recipient));

        console.log(await this.recipientTracker.deltaWithFees());
    ...
    context('when sender contract has funds', function () {
      const funds = ether('1');
      beforeEach(async function () {
        const [one, two, sender] = await ethers.getSigners();
        await sendEths(sender, this.mock.address, funds);
        console.log(await ethers.provider.getBalance(this.mock.address));
      });
